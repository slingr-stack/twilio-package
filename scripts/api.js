...
function handleRequestWithRetry(requestFn, options, callbackData, callbacks) {
    try {
        ...
        sys.logs.info("[twilio] Handling request..."+ JSON.stringify(error));
    }
}
...
exports.get = function(path, httpOptions, callbackData, callbacks) {
    ...
    return httpService.get(Twilio(options), callbackData, callbacks);
};
...
exports.post = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.post(Twilio(options), callbackData, callbacks);
};
...
exports.put = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.put(Twilio(options), callbackData, callbacks);
};
...
exports.patch = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.patch(Twilio(options), callbackData, callbacks);
};
...
exports.delete = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.delete(Twilio(options), callbackData, callbacks);
};
...
exports.head = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.head(Twilio(options), callbackData, callbacks);
};
...
exports.options = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.options(Twilio(options), callbackData, callbacks);
};
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[twilio] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[twilio] Get property: '+property);
...
function setApiUri(options) {
    ...
    let API_URL = config.get("TWILIO_API_BASE_URL");
    options.url = API_URL + url;
    sys.logs.debug('[twilio] Set url: ' + options.path + "->" + options.url);
    return options;
}

function setRequestHeaders(options) {
    let headers = options.headers || {};
    headers = mergeJSON(headers, {"Content-Type": "application/x-www-form-urlencoded"});
    options.headers = headers;
    return options;
}

function setAuthorization(options) {
    sys.logs.debug('[twilio] Setting basic authorization');
    let authorization = options.authorization || {};
    authorization = mergeJSON(authorization, {
        type: "basic",
        username: config.get("accountSid"),
        password: config.get("authToken")
    });
